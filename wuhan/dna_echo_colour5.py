# dna_echo_colour_debug_full.py
# --------------------------------------------------------------
# GPU-accelerated double φ-spiral with full per-vertex color persistence.
# Subsequent geometries properly colored along entire spiral.
...
# ---------- PERSISTENT STRAND DATA ----------
s1_full = np.zeros((0,3), dtype=np.float32)
s2_full = np.zeros((0,3), dtype=np.float32)
colors_s1_full = np.zeros((0,4), dtype=np.float32)
colors_s2_full = np.zeros((0,4), dtype=np.float32)
...
def update(ev):
    ...
    global s1_full, s2_full, colors_s1_full, colors_s2_full
    frame += 1

    # ---- spiral growth ----
    N = 50  # smaller batch per frame to accumulate
    t_start = len(s1_full)
    t_end = t_start + N
    t = np.linspace(t_start, t_end, N, endpoint=False)

    s1_new = np.zeros((N,3), dtype=np.float32)
    s2_new = np.zeros((N,3), dtype=np.float32)
    colors_s1_new = np.zeros((N,4), dtype=np.float32)
    colors_s2_new = np.zeros((N,4), dtype=np.float32)
    ...
    for i, tt in enumerate(t):
        ...
        s1_new[i] = [r*np.cos(theta)*np.cos(a1) - r*np.sin(theta)*np.sin(a1),
                     r*np.sin(theta)*np.cos(a1) + r*np.cos(theta)*np.sin(a1),
                     z]
        s2_new[i] = [r*np.cos(theta)*np.cos(a2) - r*np.sin(theta)*np.sin(a2),
        ...
        rgba_a = rgba.copy()
        rgba_a[3] *= a
        colors_s1_new[i] = rgba_a
        colors_s2_new[i] = rgba_a

    # Append new data to full arrays
    s1_full = np.vstack([s1_full, s1_new])
    s2_full = np.vstack([s2_full, s2_new])
    colors_s1_full = np.vstack([colors_s1_full, colors_s1_new])
    colors_s2_full = np.vstack([colors_s2_full, colors_s2_new])

    # Update strands
    strand1.set_data(pos=s1_full, color=colors_s1_full, width=2.0)
    strand2.set_data(pos=s2_full, color=colors_s2_full, width=2.0)

    # ---- emergence of new shapes ----
    cur_dim = min(int((t_end * speed_factor) // period), 7)
    if frame % 20 == 0 and (len(emerged) == 0 or cur_dim != emerged[-1]):
        ...
        idx_start = int((cur_dim / 8.0) * len(s1_full))
        step = max(1, len(s1_full) // (8*verts))
        idx = np.arange(idx_start, idx_start + verts*step, step)[:verts]
        idx = idx[idx < len(s1_full)]
...
if __name__ == "__main__":
    print("Starting app with persistent full-color spiral...")
    app.run()
