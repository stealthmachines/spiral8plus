"""
FASTA-Driven φ-Harmonic Spiral in a Body-Pressure Vessel
with φ-Octave Rungs & Lattice Links
--------------------------------------------------------
Run: python fasta_phi_spiral_vessel_links.py
Requirements: pip install vispy pyqt6 numpy
Everything is genome-driven: strands, rungs, echoes, organelles.
Soft spherical vessel confines the DNA lattice.
"""
...
def apply_vessel_pressure(points):
    """Apply soft spherical vessel pressure to Nx3 points"""
...
def update(ev):
    global tick, write_ptr, filled, accum_s1, accum_s2, organelles, lattice_links
    ...
    if (h[0]%50)==0:
        ...
        # Also store center for lattice links
    ...
    # ----------------- φ-OCTAVE LATTICE LINKS -----------------
    if len(centers)>1:
        # draw line from previous center to current
        last_center = centers[-2]
        current_center = centers[-1]
        lattice_lines = np.vstack([last_center, current_center])
        lattice_lines = apply_vessel_pressure(lattice_lines)
        link_lines.set_data(pos=lattice_lines.astype(np.float32))
