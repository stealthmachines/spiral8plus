...
def update(ev):
    ...
    # ---- DNA snippet ----
    dna_seq = "ATGCGTAC"  # your small DNA snippet
    base_map = {'A': 5, 'T': 2, 'G': 4, 'C': 1}  # maps nucleotide -> geometry index (1-based)
    base_idx = (frame // 20) % len(dna_seq)
    cur_base = dna_seq[base_idx]
    cur_dim = base_map[cur_base] - 1  # zero-index for geometries

    for tt in t:
        _, _, col, _, alpha, _ = geometries[cur_dim]

        r = np.exp(alpha * (tt % period))
        theta = tt * 2 * np.pi / period

        # strand 1
        a1 = np.radians(angles[cur_dim])
    ...
    # ---- emerge new rung for the DNA base every 20 frames ----
    if frame % 20 == 0:
        if len(emerged) == 0 or cur_dim != emerged[-1]:
            ...
            centers.append(cen)
            # label with geometry + DNA base
            lbl = Text(f"{dim}D: {name}\n{note}\n{cur_base}", pos=cen + [0,0,0.3],
