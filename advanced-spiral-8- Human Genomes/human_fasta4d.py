"""
FASTA-Driven φ-Harmonic Spiral with Recursive Octaves
-----------------------------------------------------
Run: python fasta_phi_spiral.py
Requirements: pip install vispy pyqt6 numpy
Everything is genome-driven: strand positions, rung vertices, echoes, and lattice links.
"""
...
max_points = 12000       # circular buffer
core_radius = 15.0
strand_sep = 0.5
...
def build_traversal(seq):
    """Return deterministic genome traversal order based on k-mer hashes"""
    k = 5
    keys = []
...
# Remove unsupported parent parameter from Text
...
def seq_triplet_values(idx):
    # Use .upper() and .get() to avoid KeyError for lower-case or unknown bases
...
def spawn_organelle(idx,center):
    ...
    mark = Markers(pos=pos.reshape(1,3).astype(np.float32),
                   face_color=rgba, edge_color=None, size=6)
...
def update(ev):
    ...
    if (ord(base)%37)==0:
        ...
        lbl=Text(f"{base}:{geom_dim}", pos=cen+np.array([0,0,0.3]),
                 color=color,font_size=10,bold=True,anchor_x='center')
        labels.append(lbl)
        centers.append(cen)
