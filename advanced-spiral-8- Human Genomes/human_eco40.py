...
MAX_POINTS = 8000
MAX_ORGANELLES = 500
NUM_BACTERIA = 12  # Dozens of ecoli spirals
...
# ---------- VISPY SETUP (must be before EColi class) ----------
...
class EColi:
    def __init__(self, genome_seq, offset=np.zeros(3)):
        ...
        self.strand1_vis = Line(pos=np.zeros((1,3)), color=(0,1,1,0.7), width=2)
        self.strand2_vis = Line(pos=np.zeros((1,3)), color=(1,0.5,0,0.7), width=2)
        self.donut_vis   = Line(pos=np.zeros((1,3)), color=(1,1,0,0.4), width=1)
    ...
    def update(self, others=[]):
        ...
        # Spawn organelles based on genome
        ...
        for org in self.organelles:
            positive = base in ['A','T']  # yin/yang FASTA-driven
...
def update(ev):
    ...
    # Rotate camera organically
    import time
    t = time.perf_counter()
    comp_view.camera.azimuth = t*12
    comp_view.camera.elevation = 15 + 8*np.sin(t*0.2)
    comp_canvas.update()
