...
# ---------- VISPY SETUP (must be before EColiCell class) ----------
...
class EColiCell:
    def __init__(self, genome_seq):
        ...
        self.strand1_vis = Line(pos=np.zeros((1,3)), color=(0,1,1,0.7), width=2, parent=comp_view.scene)
        self.strand2_vis = Line(pos=np.zeros((1,3)), color=(1,0.5,0,0.7), width=2, parent=comp_view.scene)
        self.donut_vis   = Line(pos=np.zeros((1,3)), color=(1,1,0,0.4), width=1.5, parent=comp_view.scene)
        self.labels = []
...
def update(ev):
    ...
    comp_view.camera.azimuth = elapsed_total * 12
    comp_view.camera.elevation = 15 + 8*np.sin(elapsed_total*0.2)
    comp_canvas.update()
