# dna_echo_colour_full.py
# --------------------------------------------------------------
# GPU-accelerated double φ-spiral with full structural richness
# and persistent multicolor along entire strands
# --------------------------------------------------------------
...
def update(ev):
    ...
    for i, tt in enumerate(t):
        ...
        # ---- assign full-length color per segment ----
        rgba = rgba_for(col)
        # Optional slight brightness gradient along strand
        t_norm = (tt % period) / period
        rgba_mod = rgba.copy()
        rgba_mod[:3] = np.clip(rgba[:3] * (0.7 + 0.3 * t_norm), 0, 1)
        colors_s1[i] = rgba_mod
        colors_s2[i] = rgba_mod
