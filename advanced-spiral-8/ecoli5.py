...
# ---------- VISPY ----------
canvas = scene.SceneCanvas(keys=None, size=(1200, 800), bgcolor='black')
view = canvas.central_widget.add_view()
view.camera = scene.TurntableCamera(fov=45, distance=50,
                                    azimuth=0, elevation=20,
                                    interactive=False)  # disable mouse
...
def update(ev):
    ...
    # ---- auto-rotate camera (no mouse) ----
    view.camera.azimuth = frame * 0.03
    view.camera.elevation = 20 + 10*np.sin(frame*0.002)
    view.camera.distance  = 50 + 5*np.sin(frame*0.001)
